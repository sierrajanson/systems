...
int read_value(int max, char *value){
	...
	if (user_success == 0) fatal_error("Invalid Command\n");    
	return count;    
}
...
int main(int argc, char **argv) {
	...
	if (length == 0) { write(STDOUT_FILENO, "OK\n", 3);close(fd);exit(0);}
	...
	while (count < length && read(STDIN_FILENO, &c, 1) > 0) {
		...
		int bytes = 0;
		while (bytes < 4096){
			int val = write(fd,buffer,4096-bytes); //is this resetting the buffer, or do i need to update it
			if (val < 0) fatal_error("Operation Failed\n");
			bytes +=val;
		}	
	...
	*/
	...
	if (read(STDIN_FILENO, &c, 1) > 0) fatal_error("Invalid Command\n");
